...
(function ($) {
    ...
    function Listbox(domelement, options) {
        var settings = $.extend({
            ...
            'searchbar':  false,
            'searchRegex': '^',
            'searchTextRegexKey' : "{#/SearchString/#}"
        }, options);
        ...
        searchbar.keyup(function () {
            ...
            if (searchQuery !== '') {
                ...
                self._list.children().each(function (index) {
                    ...
                    var regexp = self._settings.searchRegex;
                    if(regexp){
                      if(regexp.indexOf(self._settings.searchTextRegexKey)){
                        regexp = regexp.replace(self._settings.searchTextRegexKey,searchQuery);
                      } else {
                        regexp += searchQuery;
                      }
                    } else {
